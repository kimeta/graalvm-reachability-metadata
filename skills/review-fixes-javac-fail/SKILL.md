...
2. Validate the diff scope.
   ...
   - Reject or request changes if the PR removes tests, disables test classes, catches and ignores the failing exception, or disables native-image behavior without isolating a test that fundamentally requires unsupported open-ended dynamic class loading.
...
- The fix disables native-image behavior without isolating a test that fundamentally requires unsupported open-ended dynamic class loading.
...
void loadsGeneratedHandlerClass() throws Exception {
    assumeFalse(
            ...
            "Generated handler classes are discovered after the native executable is built");
