...
3. Review the test source.
   ...
   - Confirm the library behavior actually runs under native image by default. Reject tests that skip or short-circuit the relevant assertions in native mode through checks on `System.getProperty("org.graalvm.nativeimage.imagecode")`, early returns, or equivalent guards.
...
- Weak PR:
  ...
  - Test logic disables or bypasses the library behavior under native image instead of using the `NativeImageSupport.isUnsupportedFeatureError(e)` catch pattern for unsupported open-ended dynamic class loading.
